...
function getPhoneRandomNumber()
    ...
    local numBase1 = math.random(555,999)
...
--  Utils
--====================================================================================
function getSourceFromIdentifier(license, cb)
    TriggerEvent("es:getPlayers", function(users)
        for k , user in pairs(users) do
            if (user.getIdentifier ~= nil and user.getIdentifier() == license) or (user.license == license) then
                cb(k)
                return
            end
        end
...
function getNumberPhone(license)
    local result = MySQL.Sync.fetchAll("SELECT users.phone_number FROM users WHERE users.license = @license", {
        ['@license'] = license
    })
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = MySQL.Sync.fetchAll("SELECT users.license FROM users WHERE users.phone_number = @phone_number", {
...
function getOrGeneratePhoneNumber (sourcePlayer, license, cb)
    local sourcePlayer = sourcePlayer
    local license = license
    local myPhoneNumber = getNumberPhone(license)
    if myPhoneNumber == '0' or myPhoneNumber == nil then
        repeat
            myPhoneNumber = getPhoneRandomNumber()
            local id = getIdentifierByPhoneNumber(myPhoneNumber)
        until id == nil
        MySQL.Async.insert("UPDATE users SET phone_number = @myPhoneNumber WHERE license = @license", { 
...
--  Contacts
--====================================================================================
function getContacts(license)
    local result = MySQL.Sync.fetchAll("SELECT * FROM phone_users_contacts WHERE phone_users_contacts.license = @license", {
        ['@license'] = license
    })
    return result
end
function addContact(source, license, number, display)
    local sourcePlayer = tonumber(source)
    MySQL.Async.insert("INSERT INTO phone_users_contacts (`license`, `number`,`display`) VALUES(@license, @number, @display)", {
        ['@license'] = license,
        ['@number'] = number,
        ['@display'] = display,
    },function()
        notifyContactChange(sourcePlayer, license)
    end)
end
function updateContact(source, license, id, number, display)
    local sourcePlayer = tonumber(source)
...
function deleteContact(source, license, id)
    local sourcePlayer = tonumber(source)
    MySQL.Sync.execute("DELETE FROM phone_users_contacts WHERE `license` = @license AND `id` = @id", {
        ['@license'] = license,
        ['@id'] = id,
    })
    notifyContactChange(sourcePlayer, license)
end
function deleteAllContact(license)
    MySQL.Sync.execute("DELETE FROM phone_users_contacts WHERE `license` = @license", {
        ['@license'] = license
    })
end
function notifyContactChange(source, license)
    local sourcePlayer = tonumber(source)
    local license = license
    if sourcePlayer ~= nil then 
        TriggerClientEvent("gcPhone:contactList", sourcePlayer, getContacts(license))
    end
end
...
AddEventHandler('gcPhone:addContact', function(display, phoneNumber)
    ...
    addContact(sourcePlayer, license, phoneNumber, display)
end)
...
AddEventHandler('gcPhone:updateContact', function(id, display, phoneNumber)
    local sourcePlayer = tonumber(source)
    local license = getPlayerID(source)
    updateContact(sourcePlayer, license, id, phoneNumber, display)
end)
...
AddEventHandler('gcPhone:deleteContact', function(id)
    local sourcePlayer = tonumber(source)
    local license = getPlayerID(source)
    deleteContact(sourcePlayer, license, id)
end)

--====================================================================================
--  Messages
--====================================================================================
function getMessages(license)
    local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.license = @license WHERE phone_messages.receiver = users.phone_number", {
         ['@license'] = license
    })
    return result
...
function addMessage(source, license, phone_number, message)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number)
    local myPhone = getNumberPhone(license)
...
function setReadMessageNumber(license, num)
    local mePhoneNumber = getNumberPhone(license)
...
function deleteAllMessageFromPhoneNumber(source, license, phone_number)
    local source = source
    local license = license
    local mePhoneNumber = getNumberPhone(license)
...
function deleteAllMessage(license)
    local mePhoneNumber = getNumberPhone(license)
...
AddEventHandler('gcPhone:sendMessage', function(phoneNumber, message)
    local sourcePlayer = tonumber(source)
    local license = getPlayerID(source)
    addMessage(sourcePlayer, license, phoneNumber, message)
end)
...
AddEventHandler('gcPhone:deleteMessageNumber', function(number)
    local sourcePlayer = tonumber(source)
    local license = getPlayerID(source)
    deleteAllMessageFromPhoneNumber(sourcePlayer,license, number)
    -- TriggerClientEvent("gcphone:allMessage", sourcePlayer, getMessages(license))
end)
