	...
	if (result) then
		...
		if count > 0 then
			if (result[1].license == player) then
				TriggerClientEvent('apart:isMine', source, result[1].license)
			else
				TriggerClientEvent('apart:isBuy', source, result[1].license)
			end
		else
...
RegisterServerEvent("apart:getPlayerisInside")
AddEventHandler('apart:getPlayerisInside', function(player,appartname)
  local source = source
...
RegisterServerEvent("apart:updateIsInside")
AddEventHandler('apart:updateIsInside', function()
  local source = source
