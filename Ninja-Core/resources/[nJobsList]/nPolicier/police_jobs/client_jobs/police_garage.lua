...
mainPoliceGarage = NativeUI.CreateMenu("Garage Police","")
_menuPoolGarage:Add(mainPoliceGarage)

local function AddPoliceVeh(pVeh)
    ...
    for i=1, #Config.Locations do
        ...
        exports.nCoreStuff:Ninja_Core_PedsText("~b~Garage Police ~w~: ~h~Votre véhicule de patrouille vous attend au garage !", 7000)  
    end
end

local function RemovePoliceVeh(entity)
	SetEntityAsMissionEntity(entity,true,true)
	Citizen.InvokeNative(0xAE3CBE5BF394C9C9, Citizen.PointerValueIntInitialized(entity))
end

local function AddMenuGarage(menu)
    local pVehSubMenu = _menuPoolGarage:AddSubMenu(menu,"~b~Vehicules","~w~Liste des vehicules")
    ...
    for _, pVeh in pairs(nConfig.Vehicles) do
        ...
        pVehSubMenu.SubMenu.OnItemSelect = function(_, _, index)
            AddPoliceVeh(nConfig.Vehicles[index].Hash)
        end
    end
end
...
Citizen.CreateThread(function ()
    while true do
      ...
      _menuPoolGarage:MouseEdgeEnabled (false);
      ...
      for k,v in pairs(nConfig.Garage) do
            if Myjob == "Police" and isInServiceCops == true then
                ...
                if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < nConfig.nDistancePlayer) then
                    if GetLastInputMethod(0) then
                        exports.nCoreStuff:Ninja_Core__DisplayHelpAlert("~INPUT_TALK~ pour accéder au ~b~Garage Police")
                    else
                        exports.nCoreStuff:Ninja_Core__DisplayHelpAlert("~INPUT_CELLPHONE_RIGHT~ pour accéder au ~b~Garage Police")
                    end
                    ...
                    if (IsControlJustReleased(0, 54) or IsControlJustReleased(0, 175)) then
                        for i=1, #Config.Locations do
                            ...
                            exports.nCoreStuff:Ninja_Core_PedsText("~b~Garage Police ~w~: ~h~Que puis-je faire pour vous ?", 7000)
                        end
                        mainPoliceGarage:Visible(not mainPoliceGarage:Visible())
                    end
                end
            end
        end
        ...
        for k,v in pairs(nConfig.RemoveCarPos) do
            ...
            if Myjob == "Police" and isInServiceCops == true then
                ...
                if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < nConfig.nDistancePlayer) then
                    if GetLastInputMethod(0) then
                        exports.nCoreStuff:Ninja_Core__DisplayHelpAlert("~INPUT_TALK~ pour ~r~ranger ~w~ votre véhicule de patrouille")
                    else
                        exports.nCoreStuff:Ninja_Core__DisplayHelpAlert("~INPUT_CELLPHONE_RIGHT~ pour ~r~ranger ~w~votre véhicule de patrouille")
                    end
                    
                    if (IsControlJustReleased(0, 54) or IsControlJustReleased(0, 175)) then
                        if ( GetPedInVehicleSeat( vehicle, -1 ) == GetPlayerPed(-1) ) then 
                            SetEntityAsMissionEntity( vehicle, true, true )
                            RemovePoliceVeh(vehicle)
                        end 
                    end
                end
            end
...
AddMenuGarage(mainPoliceGarage)
_menuPoolGarage:MouseEdgeEnabled (false);
